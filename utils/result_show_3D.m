...
hold on; plot3(source_vertex(landmark_source_index,1), source_vertex(landmark_source_index,2), source_vertex(landmark_source_index,3), 'yo'); hold off;
...
hold on; plot(flat_source_vertex(landmark_source_index,1), flat_source_vertex(landmark_source_index,2), 'yo'); hold off;
...
plot(source_vertex_reg(landmark_source_index,1), source_vertex_reg(landmark_source_index,2), 'bo','MarkerSize',5);
plot(flat_target_vertex(landmark_target_index,1), flat_target_vertex(landmark_target_index,2), 'ro', 'MarkerSize',5);
title('Overlaid view');
hold off;
...
subplot(1,2,2); gpp_plot_mesh(source_face, source_vertex, correspondence_mask); colormap parula; title('Correspondence region');
...
subplot(1,2,2); plot(1:length(intensity_err), intensity_err); title('Intensity Error');
