...
quiver(source_vertex(sub_ind,1), source_vertex(sub_ind,2),...
    displace_2D(sub_ind,1), displace_2D(sub_ind,2),0, 'Color',...
    [216,108,112]/255, 'LineWidth', 0.2,...
    'MaxHeadSize', 0.05); colorbar;
hold off; title('Displacement field visualization');
figure(5);
% subplot(1,2,1); 
gpp_plot_mesh(source_face_reg, source_vertex, abs(source_intensity - target_intensity_reg)); colormap parula; title('Registered w/ curvature diff');
